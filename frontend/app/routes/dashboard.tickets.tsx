      ...
      <div className="max-w-screen-xl mx-auto px-6 py-12">
        ...
        {tickets.length === 0 ? (
          <div className="text-center py-20">
            ...
            <h2 className="text-xl font-semibold mb-2">{t("tickets.emptyTitle")}</h2>
            <p className="text-iron-grey mb-6">
               {t("tickets.emptyDesc")}
            </p>
            <Link to="/">
                  <Button variant="secondary">{t("tickets.browseEvents")}</Button>
            </Link>
          </div>
        ) : (
              ...
              >
                <div className="flex justify-between items-start mb-3">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    >
                      {tk.is_redeemed ? t("tickets.used") : t("tickets.active")}
                    </span>
                ...
                <div className="flex items-center gap-4 text-xs text-pastel-purple">
                  <span className="font-mono">{tk.code}</span>
                  <span className="flex items-center gap-1">
                    <Calendar className="w-3 h-3" />
                    {new Date(
                      tk.transaction_item.transaction.event.start_time,
                    ).toLocaleDateString("id-ID")}
                  </span>
                  {tk.transaction_item.transaction.event.location && (
                    <span className="flex items-center gap-1">
